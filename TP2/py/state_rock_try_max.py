...
class State:
    ...
    ###ROCK
    def nb_rocks_blocking(self, rh, car_selected, collision_pos, depth):
        nb_cars_blocked = -999999
        ...
        if rh.horiz[car_selected]:
            ...
            if self.pos[car_selected] - mvts_left >= 0:
                # print("Car :", rh.color[car_selected], " mtvs_left: ", mvts_left)
                nb_cars_blocked = max(nb_cars_blocked, self.calculate_blocking_and_blocked_cars_rocks(rh, car_selected, -mvts_left, depth))
                nb_cars_blocked += mvts_left
            
            # Move right
            if self.pos[car_selected] + rh.length[car_selected] - 1 + mvts_right <= 5:
                # print("Car :", rh.color[car_selected], " mtvs_right: ", mvts_right)
                nb_cars_blocked = max(nb_cars_blocked, self.calculate_blocking_and_blocked_cars_rocks(rh, car_selected, mvts_right, depth))
                nb_cars_blocked += mvts_right
            ...
            if self.pos[car_selected] - mvts_up >= 0:
                # print("Car :", rh.color[car_selected], " mtvs_up: ", mvts_up)
                nb_cars_blocked = max(nb_cars_blocked, self.calculate_blocking_and_blocked_cars_rocks(rh, car_selected, -mvts_up, depth))
                nb_cars_blocked += mvts_up

            # Move down
            if self.pos[car_selected] + rh.length[car_selected] - 1 + mvts_down <= 5:
                # print("Car :", rh.color[car_selected], " mvts_down: ", mvts_down)
                nb_cars_blocked = max(nb_cars_blocked, self.calculate_blocking_and_blocked_cars_rocks(rh, car_selected, mvts_down, depth))
                nb_cars_blocked += mvts_down
                
        return nb_cars_blocked

    def calculate_blocking_and_blocked_cars_rocks(self, rh, car_selected, mvts, depth):
        nb_cars_blocking = 0
        nb_cars_blocked = 0
        ...
        for car in overlapping_cars:
            # print("Overlapping car : ", rh.color[car])   
            nb_cars_blocking += self.nb_rocks_blocking(rh, car, self.find_overlapping_collision(rh, car_selected, mvts, car), depth + 1)
            # print("Back from Overlapping car : ", rh.color[car])   
